...
IM_Serializer::IM_Serializer() : mCompatibilityMode(false) { mPrefixStack.push(""); }

bool IM_Serializer::isSerializedFile(const std::string &filename) {
...
std::vector<std::string> IM_Serializer::getKeys() const {
...
IM_Serializer::IM_Serializer(const std::string &filename, bool compatibilityMode, const ProgressCallback &progress)
    ...
    p_ply ply = ply_open(filename.c_str(), message_cb, 0, nullptr);
    if (!ply)
        throw std::runtime_error("IM_Serializer: unable to open PLY file \"" + filename + "\"!");

    if (!ply_read_header(ply)) {
        ply_close(ply);
        throw std::runtime_error("IM_Serializer: unable to open PLY header of \"" + filename + "\"!");
    }
    ...
    try {
        ...
        while ((element = ply_get_next_element(ply, element)) != nullptr) {
            ...
            while ((property = ply_get_next_property(element, property)) != nullptr) {
                ...
                if (!ply_set_read_cb(ply, element_name, property_name, rply_element_cb, state, rows++)) {
                    ...
                    throw std::runtime_error(
                        "IM_Serializer: could not register read callback for " + std::string(element_name) +
            ...
            if (fail)
                throw std::runtime_error("IM_Serializer: unsupported data format in \"" + filename + "\"!");
...
void IM_Serializer::write(const std::string &filename, const ProgressCallback &progress) {
    auto message_cb = [](p_ply ply, const char *msg) { cerr << "rply: " << msg << endl; };
...
size_t IM_Serializer::totalSize() const {
    size_t result = 0;
...
bool IM_Serializer::diff(const IM_Serializer &other) const {
...
std::ostream &operator<<(std::ostream &os, const IM_Serializer &state) {
    os << "IM_Serializer[";
    bool first = true;
    for (auto const &kv : state.mData) {
        const IM_Serializer::Variant &v = kv.second;
        if (!first)
            cout << ",";
        first = false;
        cout << endl;
        ...
        #define IMPLEMENT(type) \
            case IM_Serializer::Variant::Type::Matrix_##type: { \
                ...
                break; \
            case IM_Serializer::Variant::Type::List_##type: \
                tname = #type "**"; \
