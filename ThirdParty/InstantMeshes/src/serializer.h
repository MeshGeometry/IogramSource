...
public:
    IM_Serializer();
    IM_Serializer(const std::string &filename, bool compatibilityMode = false,
    ...
    bool diff(const IM_Serializer &serializer) const;
    ...
    #define MISSING_KEY(key) \
        if (mCompatibilityMode) {\
            cerr << "Warning: IM_Serializer: could not find element \"" + mPrefixStack.top() + key + "\"!" << endl; \
            return false; \
        } else { \
            throw std::runtime_error("IM_Serializer: could not find element \"" + mPrefixStack.top() + key + "\"!"); \
        }
    ...
    #define IMPLEMENT(type, btype) \
        ...
        bool get(const std::string &key, type &value) const { \
            ...
            if (it->second.type_id != Variant::Type::Matrix_##btype) \
                throw std::runtime_error("IM_Serializer: element \"" + mPrefixStack.top() + key + "\": type mismatch!"); \
            if (it->second.matrix_##btype->size() != 1) \
                throw std::runtime_error("IM_Serializer: element \"" + mPrefixStack.top() + key + "\": size mismatch!"); \
    ...
    friend std::ostream &operator<<(std::ostream &os, const IM_Serializer &state);
