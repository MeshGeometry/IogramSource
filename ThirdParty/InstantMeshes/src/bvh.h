...
public:
    ...
    bool rayIntersect(IM_Ray ray) const;

    bool rayIntersect(IM_Ray ray, uint32_t &idx, Float &t, Vector2f *uv = nullptr) const;
...
protected:
    bool rayIntersectTri(const IM_Ray &ray, uint32_t i, Float &t, Vector2f &uv) const;
    bool rayIntersectDisk(const IM_Ray &ray, uint32_t i, Float &t) const;
