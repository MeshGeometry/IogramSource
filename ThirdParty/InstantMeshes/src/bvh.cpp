...
bool BVH::rayIntersect(IM_Ray ray, uint32_t &idx, Float &t, Vector2f *uv) const {
    if (mNodes.empty())
        return false;
...
bool BVH::rayIntersect(IM_Ray ray) const {
    if (mNodes.empty())
        return false;
...
bool BVH::rayIntersectTri(const IM_Ray &ray, uint32_t i, Float &t, Vector2f &uv) const {
...
bool BVH::rayIntersectDisk(const IM_Ray &ray, uint32_t i, Float &t) const {
